...
Are those on the job hunt supposed to be demure and welcome asymmetric information? Inferring the salary as best they can, and go through the whole application process, only to find out at the end. This is time-consuming and can be difficult if the specific role is niche or is in a new field. A web application link will follow where HR and job searchers in Data Science can copy and paste the description. Perhaps some job description authors will use the model to deliberately bely true salaries down the line…
