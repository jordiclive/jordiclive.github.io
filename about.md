---
title: About
layout: default
published: true
---
### N.B.

I have nearly finished my **data science with machine learning** course and I am going to be adding entries concerning the projects I have worked on, in and out of the course. See [projects](https://jordiclive.github.io/projects.html) for a short précis of each completed project. 

As I have some spare time over the next two weeks, write-ups will appear. I am currently 14th/3228 on the [public LB](https://www.kaggle.com/c/elo-merchant-category-recommendation/leaderboard) of my first Kaggle competition—some ML techniques were required (I promise!).

Additionally, I am hoping to be a teaching assistant for the same data science course, starting in [March](https://generalassemb.ly/education/data-science-immersive). I would lead full days, lecturing ML topics followed by guiding labs (usually undertaken in Jupyter Notebooks), as well as provide asssistance to students on a day-to-day basis. I believe this would be a good step in my development, preparing me for study and research in September 2019—I would be able to build and consolidate my machine learning knowledge, whilst using state-of-the-art libraries, something generally pushed for by the course.

- - -

# About 

<img src="{{site.baseurl}}/0.jpg" alt="me" width="200"/>

I'm **Jordi**, formally Jordan. 

I have set up this blog primarily to showcase my intent on studying machine learning (ML)—I have applied to Master’s programs in the subject and wish to write up my first projects and thoughts. 

I became enamoured when studying Data Science for 12 weeks.

Secondarily I have always wished to blog: I used to journal anyway, and I was always inspired by one [idealist’s blog](https://www.thrivenotes.com/) who shared my interests and naivety. I stumbled upon their thoughts in 2011, quite serendipitously, or so I thought, rather owing to some obscure search matches. They were unconscious of my interest until I made contact many years later. The point is that that promise; that somebody, somewhere, may derive some meaning, makes the action of marshalling ones thoughts (which is good for you anyway!) that much better. Obviously it is nearly 100% self-serving and it is well noted that many people have blogs and many people are not interesting.

My (pseudo)intellectual interests are: learning, teaching, ageism, meta-awareness, intelligence, neurotechnology, gerontology, scientist biographies, transhumanism and investments. 

Things I enjoy are mountaineering, tennis, trail-running and ski-touring.

With regards to the platform, it is free! Whilst being free from advertising.

- - -

## Contact

Feel free to [email me](mailto:jordiclive@protonmail.com).

If you need to contact me privately, please use my [PGP key](/jordi.asc). My key’s fingerprint is:

    62BD 5F6D 6523 79E5 5911  519C 206F CDB8 E520 427C
